...
MODEL_DIR = os.path.join("Models", "NN_Single_frequency")
WEIGHTS_PATH = os.path.join(MODEL_DIR, "NN_Single_frequency_Weights.csv")
BIASES_PATH = os.path.join(MODEL_DIR, "NN_Single_frequency_Biases.csv")

# Input/Output Data Paths
# This path can be configured to point to your new input data file (.csv or .xlsx)
INPUT_DATA_PATH = os.path.join("Inputs", "input_data_1h.csv")
OUTPUT_CSV_PATH = os.path.join("Outputs", "predictions_1h.csv")
...
def load_input_data(file_path, device):
    ...
    According to specifications, column 0 is the Real part and column 1 is the Imaginary part.
    ...
    Returns:
        torch.Tensor: Preprocessed input tensor of shape (N, 2) on the correct device.
    """
    ...
    # Extract the first two columns (Real and Imaginary parts of the fundamental harmonic)
    raw_values = df.iloc[:, :2].values
...
def main():
    ...
    # Dropout rate is set to 0.005 for inference as configured in the original script
    model = models.SingleFrequencyModel(layer_dims=dense_dimensions, dropout=0.005)
    ...
    predictions_df = pd.DataFrame(predictions_np, columns=["X1", "Y1"])
